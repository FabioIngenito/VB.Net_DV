...
Friend Module mdlCalculo
    ...
    Public Function Mod_dig11(cVariavel As String) As String
        Dim lRetorno As String = "0"
        Dim nSoma As Short = 0
        Dim nMult As Short = 2
        Dim nIndice As Short

        For nIndice = Len6(cVariavel) To 1 Step -1
            nSoma += (Asc(Mid(cVariavel, nIndice, 1)) - 48) * nMult
    ...
    ' CONVERSAO_INFO (#0551): O parâmetro 'strNumero' não foi modificado ou passado para um método que o modifique. Considere remover o ByRef.
    Public Function CalculoDV11(ByRef strNumero As String) As String
        'declara as variáveis
        ' CONVERSAO_INFO (#0561): O símbolo 'intcontador' foi definido sem o uso explícito da cláusula "As".
        Dim intcontador As Object = Nothing
        ' CONVERSAO_INFO (#0561): O símbolo 'intnumero' foi definido sem o uso explícito da cláusula "As".
        Dim intnumero As Object = Nothing
        ' CONVERSAO_INFO (#0561): O símbolo 'intTotalNumero' foi definido sem o uso explícito da cláusula "As".
        Dim intTotalNumero As Object = Nothing
        ' CONVERSAO_INFO (#0561): O símbolo 'intMultiplicador' foi definido sem o uso explícito da cláusula "As".
        Dim intMultiplicador As Object = Nothing
        Dim intResto As Short

        ' se nao for um valor numerico sai da função
        If Not IsNumeric6(strNumero) Then
            Return ""
        End If
        ...
        For intcontador = 1 To Len6(strNumero)
            ...
            intMultiplicador = If(intMultiplicador > 2, intMultiplicador - 1, 9)

        Next

        'calcula o resto da divisao do total por 11
        intResto = CInt(intTotalNumero) Mod 11

        'verifica as exceções (intResto = 10 então DV = "X")

        If intResto < 10 Then
            Return Trim(Str6(intResto))
        Else
            Return "X"
        End If
